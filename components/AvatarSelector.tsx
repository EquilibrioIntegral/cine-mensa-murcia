        ...
        <div className="bg-black/20 p-4 rounded-xl border border-gray-700 min-h-[200px] max-h-[300px] overflow-y-auto custom-scrollbar">
            ...
            <form onSubmit={handleSearch} className="flex gap-2 mb-4 sticky top-0 z-10">
                <input 
                    ...
                    placeholder={tmdbToken ? "Buscar actor/director..." : "Token TMDB requerido"}
                    disabled={!tmdbToken}
                    className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:border-cine-gold outline-none disabled:opacity-50"
                />
            ...
            {searchResults.length === 0 && !searching && (
                <div className="text-center pt-8 opacity-50">
                    <p className="text-gray-400 text-xs">Busca tu estrella favorita (ej: Brad Pitt, Tarantino...)</p>
                </div>
            )}
