    ...
    <div className="bg-black/60 border border-green-500/30 rounded-xl p-4 mb-8 shadow-[0_0_20px_rgba(34,197,94,0.1)] relative overflow-hidden animate-fade-in">
        ...
        <div className="flex flex-wrap gap-4 relative z-10">
            {onlineUsers.map(user => (
                <div key={user.id} className="flex items-center gap-2 bg-black/40 pr-3 rounded-full border border-green-900/50">
                    <div className="relative">
                        <img src={user.avatarUrl} alt={user.name} className="w-8 h-8 rounded-full border border-green-700/50" />
